var runtime = (function() {
  ...
  var getTopURLPromise = new Promise(function(resolve, reject) {
    if (window === top) {
      ...
      RUNTIME("getTopURL", null, function(rs) { resolve(rs.url); });
    }
  });

  self.on = function(message, cb) { _handlers[message] = cb; };

  self.updateHistory = function(type, cmd) {
    var prop = type + 'History';
    RUNTIME('getSettings', {key : prop}, function(response) {
      var list = response.settings[prop] || [];
      var toUpdate = {};
      if (cmd.constructor.name === "Array") {
        toUpdate[prop] = cmd;
        RUNTIME('updateSettings', {settings : toUpdate});
      } else if (cmd.trim().length && cmd !== ".") {
        list = list.filter(function(
            c) { return c.trim().length && c !== cmd && c !== "."; });
        list.unshift(cmd);
        if (list.length > 50) {
          list.pop();
        }
  ...
  self.getTopURL = function(
      cb) { getTopURLPromise.then(function(url) { cb(url); }); };
  ...
  self.getCaseSensitive = function(query) {
    return self.conf.caseSensitive ||
           (self.conf.smartCase && /[A-Z]/.test(query));
  };
